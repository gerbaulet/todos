...
const TIMELINE_LABEL_WIDTH=190;
...
function timelineMarkerWidth(task){
  ...
  const width=Math.min(230,Math.ceil(marker.getBoundingClientRect().width));
  marker.remove();
  return width;
}
...
function initials(name){return String(name||"?").split(/\s+/).filter(Boolean).slice(0,2).map(part=>part[0]).join("").toLocaleUpperCase("de")}
function renderTimeline(){
  ...
  if(window.matchMedia("(max-width: 700px)").matches)return renderTimelineList(tasks);
  const px=DAY[state.settings.zoom]||18,start=new Date();start.setHours(12,0,0,0);start.setDate(start.getDate()-45);const days=500,width=TIMELINE_LABEL_WIDTH+days*px;
  let months="";const cursor=new Date(start);cursor.setDate(1);if(cursor<start)cursor.setMonth(cursor.getMonth()+1);while(dayOffset(isoLocal(cursor),start)<days){const left=TIMELINE_LABEL_WIDTH+dayOffset(isoLocal(cursor),start)*px,next=new Date(cursor);next.setMonth(next.getMonth()+1);months+=`<div class="tl-month" style="left:${left}px;width:${Math.max(1,(next-cursor)/86400000)*px}px">${cursor.toLocaleDateString("de-DE",{month:"long",year:"numeric"})}</div>`;cursor.setMonth(cursor.getMonth()+1)}
  let markerPositions={},rows="",rowOffset=0;
  for(const name of assignees){
    const same=entries.filter(x=>(x.task.assignee||"Ohne Bearbeiter")===name).map(entry=>{const range=(dayOffset(entry.end,start)-dayOffset(entry.start,start)+1)*px;return {...entry,left:TIMELINE_LABEL_WIDTH+dayOffset(entry.start,start)*px,width:entry.precision==="exact"?timelineMarkerWidth(entry.task):Math.max(8,range)}}).sort((a,b)=>a.left-b.left||a.task.id-b.task.id||a.recommended-b.recommended),laneEnds=[];
    let markers="";
    for(const item of same){
      ...
      const isOverdue=!t.completed&&item.end<localDate();
      markers+=`<button class="tl-marker precision-${item.precision} ${!item.recommended&&t.is_milestone?"milestone-marker":""} ${item.recommended?"recommended-marker":""} ${t.completed?"done":""} ${isOverdue?"overdue":""}" ${item.recommended?"":'draggable="true"'} data-id="${t.id}" style="left:${item.left}px;top:${top}px;width:${item.width}px;--color:${assigneeColor(t.assignee)}" title="${esc(t.title||"(ohne Titel)")} – ${esc(item.label)}">${isOverdue?'<span class="marker-alert" aria-label="Überfällig">!</span>':""}${!item.recommended&&t.is_milestone&&item.precision!=="exact"?'<span aria-hidden="true">◆ </span>':""}<span class="marker-title">${esc(t.title||"(ohne Titel)")}</span></button>`;
    }
    const rowHeight=Math.max(62,12+laneEnds.length*32),color=assigneeColor(name==="Ohne Bearbeiter"?null:name);rows+=`<div class="tl-row" style="height:${rowHeight}px"><div class="tl-label"><span class="assignee-avatar" style="--color:${color}">${esc(initials(name))}</span><span>${esc(name)}</span></div>${markers}</div>`;rowOffset+=rowHeight;
  }
  const todayX=TIMELINE_LABEL_WIDTH+dayOffset(localDate(),start)*px;let svg="";if(state.settings.showDependencies){for(const t of tasks)for(const d of t.dependencies){const target=markerPositions[t.id],source=markerPositions[d.depends_on_task_id];if(source&&target){const points=source.center<=target.center?{x1:source.right,x2:target.left}:{x1:source.left,x2:target.right};svg+=`<line x1="${points.x1}" y1="${source.y}" x2="${points.x2}" y2="${target.y}" stroke="#64748b" stroke-width="1.5" marker-end="url(#arrow)"><title>${esc(depLabel(d))}</title></line>`}}}
  ...
  $("#timeline").innerHTML=`<div class="tl-canvas zoom-${state.settings.zoom}" style="width:${width}px;--week-width:${px*7}px;--label-width:${TIMELINE_LABEL_WIDTH}px"><div class="tl-months">${months}</div><div class="today-line" style="left:${todayX}px"><span>Heute</span></div>${rows}<svg class="dep-lines" width="${width}" height="${rowOffset}"><defs><marker id="arrow" markerWidth="7" markerHeight="7" refX="6" refY="3" orient="auto"><path d="M0,0 L0,6 L6,3 z" fill="#64748b"/></marker></defs>${svg}</svg><div class="undated"><strong>Ohne Termin</strong><div class="undated-list">${undated||'<span class="muted">Keine Aufgaben</span>'}</div></div></div>`;
  ...
  scroller.ondragover=e=>e.preventDefault();scroller.ondrop=e=>{e.preventDefault();const id=+e.dataTransfer.getData("text/plain"),task=state.tasks.find(t=>t.id===id),rect=scroller.getBoundingClientRect(),x=e.clientX-rect.left+scroller.scrollLeft-TIMELINE_LABEL_WIDTH,index=Math.round(x/px),d=new Date(start);d.setDate(d.getDate()+index);if(task?.due_type)updateTask(id,dueAtDate(task.due_type,d))};
}
function renderTimelineList(tasks){
  const today=localDate(),data=[...tasks].sort((a,b)=>(effectiveDueStart(a)||"9999").localeCompare(effectiveDueStart(b)||"9999")||a.id-b.id);
  $("#timeline").innerHTML=`<div class="timeline-list"><header><span>Terminübersicht</span><strong>${data.length} ${data.length===1?"Aufgabe":"Aufgaben"}</strong></header>${data.map(t=>{const date=effectiveDueStart(t),overdue=!t.completed&&date&&((t.due_end||date)<today),recommended=!t.due_display&&date;return `<button type="button" class="timeline-list-item ${t.completed?"done":""}" data-id="${t.id}"><span class="task-checkbox" aria-checked="${t.completed}" aria-hidden="true"><span>${t.completed?"✓":""}</span></span><span class="timeline-list-copy"><strong>${t.is_milestone?'<span aria-label="Meilenstein">◆ </span>':""}${esc(t.title||"(ohne Titel)")}</strong><small>${esc(t.assignee||"Ohne Bearbeiter")}${t.project?` · ${esc(t.project)}`:""}</small></span><span class="due-badge ${overdue?"due-overdue":""}">${overdue?'<b aria-hidden="true">!</b> ':""}${esc(t.due_display||(recommended?`Empfohlen ${dateLabel(date)}`:"Ohne Termin"))}</span></button>`}).join("")||'<div class="empty"><span class="empty-icon" aria-hidden="true">✓</span><strong>Keine Aufgaben in dieser Ansicht</strong></div>'}</div>`;
  $$(".timeline-list-item").forEach(item=>item.onclick=()=>openEditor(+item.dataset.id));
}
...
function bind(){
  ...
  document.addEventListener("click",e=>$$('#tools details[open], .timeline-tools details[open]').forEach(menu=>{if(!menu.contains(e.target))menu.open=false}));
  ...
  $("#help").onclick=()=>$("#shortcuts").showModal();$("#close-help").onclick=()=>$("#shortcuts").close();$("#edit-form").onsubmit=saveEditor;$("#cancel-edit").onclick=$("#close-editor").onclick=()=>$("#editor").close();
  ...
  let resizeTimer;window.addEventListener("resize",()=>{clearTimeout(resizeTimer);resizeTimer=setTimeout(()=>{if(state.settings.view==="timeline")renderTimeline()},150)});
